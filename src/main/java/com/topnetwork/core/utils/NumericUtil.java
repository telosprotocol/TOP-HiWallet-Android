...
//Handles private key, public key, address Numeric utility class
public class NumericUtil {
    ...
    //Hex16进制
    public static String bytesToHexWithPrefix(byte[] input) {
        StringBuilder stringBuilder = new StringBuilder();
        if (input.length == 0) {
            return "";
        }
    ...
    public static String bytesToHex1(byte[] bytes) {
        ...
        int a = 0;
        int index = 0;
        for (byte b : bytes) { // Conversion using division and redundancy
            if (b < 0) {
                a = 256 + b;
            } else {
                a = b;
